...
JSONArray userpowerm1=(JSONArray)session.getAttribute("userpowerm");
	...
	String receive1 = StringUtil.NVL((String)request.getAttribute("receive1"));
	...
	String remark2 = StringUtil.NVL((String)request.getAttribute("remark2"));
...
jQuery("#list").jqGrid({	  
	  url: 'document.do?method=documentSendList<%=sparam%>',
	<%if(!"".equals(pgno)){%>
		page : <%=pgno%>,
	<%}%>
	  datatype: "json",
      ...
      width:'1100',
      height:'365',
      colNames: [ '발송일','부서명','부서코드','문서번호','수신자','제목','첨부물','비고1','비고2','발송자'],
      colModel: [
			...
			{name:'detcodename', index:'detcodename',  width: 80, editable: false, align: 'center'},
   			...
   			{name:'receive',     index:'receive',      width: 80, editable: false, align: 'left', formatter:decode11},
   			...
   			{name:'remark1',     index:'remark1',      width: 150,editable: false, align: 'left', formatter:decode11},	
   			{name:'remark2',     index:'remark2',      width: 150,editable: false, align: 'left', formatter:decode11},	
...
document.sForm.receive1.value    = "<%=receive1%>";
...
document.sForm.remark2.value    = "<%=remark2%>";
});
...
function goSelect(rowid,iCol){
	...
	if( gr != null ) {
		...
		document.sForm.receive1.value    = list.receive;
		document.sForm.title1.value      = list.title;
		document.sForm.attach1.value     = list.attach;
		document.sForm.remark1.value     = list.remark1; 
		document.sForm.remark2.value     = list.remark2;
	}
}
...
function goInsert(){
	...
	var receive1   	   = sForm.receive1.value;
    ...
    if ( year_seq1 == "" ) { 
    	document.sForm.action = "document.do?method=insert_send&g_name="+g_name;
    	document.sForm.submit();  
    }else { 
        sparam+="&s_pgno="+$('#list').getGridParam('page');
    	document.sForm.action = "document.do?method=update_send&year_seq1="+year_seq1+"&g_name="+g_name+sparam;
    	document.sForm.submit();
	 }	
}
...
function goSearch(form,intPage){
    ...
    if(sForm.receive1.value        !="")parm+="&receive1="     +escape(encodeURIComponent(sForm.receive1.value)); 
    ...
    if(sForm.remark2.value         !="")parm+="&remark2="      +escape(encodeURIComponent(sForm.remark2.value));
    sparam="";
    ...
    if(sForm.receive1.value        !="")sparam+="&s_receive1="     +escape(encodeURIComponent(sForm.receive1.value)); 
    ...
    if(sForm.remark2.value         !="")sparam+="&s_remark2="      +escape(encodeURIComponent(sForm.remark2.value));
	$('#list').jqGrid('clearGridData');
	jQuery("#list").setGridParam({url:"document.do?method=documentSendList&parm="+parm}).trigger("reloadGrid");
}
...
function excelDown(){	
	...
	document.sForm.action="document.do?method=documentsendDown&parm="+parm+"&g_name="+g_name+"&g_id="+g_id;
	document.sForm.submit();
}
...
<!-- 중복처리방지 토큰 처리  끝-->	  
	  ...
	  <table class="ctable_03" cellspacing="0" summary="문서">                 			 
             <tr>
              	<td class="nobg" align="center">발송일</td>               			   
			 	...
			 	<td class="nobg1"><input type="text" name="inqday1" id="datepicker1" class="input" style=padding-top:3px  style=padding-bottom:3px value='' size="12" align="center"/>						 
								~ <input type="text" name="inqday2" id="datepicker2" class="input" style=padding-top:3px  style=padding-bottom:3px value='' size="12" align="center"/></td>		  
              </tr> 
              <tr>             
              	...
              	<td width="24%">
               	...
               	<td class="alt" align="center">발송자</td>
               	<td><input class="readOnly1" type="text"  id="name" name="register1" size="15" value="<%=g_name %>" readonly/></td>      			  
             </tr>	
			 <tr>
               <td class="alt1" align="center">수신자</td>
               <td colspan="5"><input type="text" id="name" name="receive1" size="147" value="" /></td>  		   			  
             </tr>
			 <tr>
			 ...
			 <tr>
               <td class="alt1" align="center">비고1</td>
               <td colspan="5"><input type="text" id="name" name="remark1" size="147" value="" /></td> 
             </tr>
             <tr>
               <td class="alt1" align="center">비고2</td>
               <td colspan="5"><input type="text" id="name" name="remark2" size="147" value="" /></td> 
             </tr>             
       </table> 
             <ul class="btnIcon_2">	             
