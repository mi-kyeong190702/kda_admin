...
<%
	...
	if(request.getAttribute("req")!=null){
		req=(Map)request.getAttribute("req");
		receipt_dt=req.get("receipt_dt1").toString();
		//year_seq=req.get("year_seq1").toString();
		doc_seq=req.get("doc_seq1").toString();
		center=req.get("center1").toString();
		run_dt=req.get("run_dt1").toString();
		send=req.get("send1").toString();		
		title=req.get("title1").toString();
		accach=req.get("accach1").toString();
		//register=req.get("register1").toString();
		remark=req.get("remark1").toString();
		reqs=req.size();
	}
	...
	String doc_seq1 = StringUtil.NVL((String)request.getAttribute("doc_seq1"));
	String center1 = StringUtil.NVL((String)request.getAttribute("center1"));
	String run_dt1 = StringUtil.NVL((String)request.getAttribute("run_dt1"));
	String send1 = StringUtil.NVL((String)request.getAttribute("send1"));
	String title1 = StringUtil.NVL((String)request.getAttribute("title1"));
	String accach1 = StringUtil.NVL((String)request.getAttribute("accach1"));
...
function init(){
	...
	if(errMsg.length>5){
		alert(errMsg);
// 		if(reqs > 0){
<%-- 			document.sForm.receipt_dt1.value = '<%=receipt_dt%>'; --%>
<%-- 			<%-- document.sForm.year_seq1.value   = '<%=year_seq%>'; -- %> --%>
<%-- 			document.sForm.doc_seq1.value    = '<%=doc_seq%>'; --%>
<%-- 			document.sForm.center1.value     = '<%=center%>'; --%>
<%-- 			document.sForm.run_dt1.value     = '<%=run_dt%>'; --%>
<%-- 			document.sForm.send1.value       = '<%=send%>';		 --%>
<%-- 			document.sForm.title1.value      = '<%=title%>'; --%>
<%-- 			document.sForm.accach1.value     = '<%=accach%>'; --%>
<%-- 			<%-- document.sForm.register1.value   = '<%=register%>'; -- %> --%>
<%-- 			document.sForm.remark1.value     = '<%=remark%>'; --%>
// 		}
		return;
	}
	if(logid==null||logid==""){
...
jQuery("#list").jqGrid({	  
	  url: 'document.do?method=documentAccpList<%=sparam%>',
	<%if(!"".equals(pgno)){%>
		page : <%=pgno%>,
	<%}%>
	  datatype: "json",
      ...
      width:'1100',
      height:'365',
      colNames: [ '접수일','접수번호','발신처','문서번호','시행일자','제목','첨부물','관리부서','부서코드','비고'],
      colModel: [
			{name:'receipt_dt',  index:'receipt_dt',   width: 80,editable:  false, align: 'center'},
   			...
   			{name:'send',        index:'send',         width: 180,editable: false, align: 'center', formatter:decode11},
   			{name:'doc_seq',     index:'doc_seq',      width: 80,editable:  false, align: 'center', formatter:decode11},
   			{name:'run_dt',      index:'run_dt',       width: 80,editable:  false, align: 'center'},
   			{name:'title',       index:'title',        width: 180,editable: false, align: 'left', formatter:decode11},
   			{name:'accach',      index:'accach',       width: 100,editable: false, align: 'left', formatter:decode11},		
   			{name:'detcodename', index:'detcodename',  width: 80,editable:  false, align: 'left'},	
   			{name:'detcode', 	 index:'detcode',      hidden:true},
   			/* {name:'register',    index:'register',     width: 80,editable:  false, align: 'left', formatter:decode11, hidden:true}, */		
   			{name:'remark',      index:'remark',       width: 120,editable: false, align: 'left', formatter:decode11}	   			
   			],
   		    rowNum:15,
   		    sortname: 'receipt_dt',
   			pager: '#pager2',
...
document.sForm.doc_seq1.value    = "<%=doc_seq1%>";
document.sForm.center1.value    = "<%=center1%>";
document.sForm.run_dt1.value    = "<%=run_dt1%>";
document.sForm.send1.value    = "<%=send1%>";
document.sForm.title1.value    = "<%=title1%>";
document.sForm.accach1.value    = "<%=accach1%>";
...
function goSelect(rowid,iCol){
	...
	if( gr != null ) {
		...
		document.sForm.doc_seq1.value    = list.doc_seq;
		document.sForm.center1.value     = list.detcode;
		document.sForm.run_dt1.value     = list.run_dt;
		document.sForm.send1.value       = list.send;		
		document.sForm.title1.value      = list.title;
		document.sForm.accach1.value     = list.accach;
...
function goInsert(){
	...
	var doc_seq1	   = sForm.doc_seq1.value;
	var run_dt1		   = sForm.run_dt1.value;
	var year_seq1	   = sForm.year_seq1.value;
	var send1   	   = sForm.send1.value;
    ...
    if ( doc_seq1 == "" ) {
    	alert("문서번호를 입력해주세요.") ;
        sForm.doc_seq1.focus();
        return ;
    } 
    /* if ( center1 == "" ) {
    	alert("부서 담당자를 입력해주세요.") ;
        sForm.center1.focus();
        return ;
    } */ 
    if ( run_dt1 == "" ) {
    ...
    if ( year_seq1 == "" ) { 
    	document.sForm.action = "document.do?method=insert_accp";
    	document.sForm.submit();  
    }else { 
        sparam+="&s_pgno="+$('#list').getGridParam('page');
    	document.sForm.action = "document.do?method=update_accp&year_seq1="+year_seq1+sparam;
    	document.sForm.submit();
	 }	
}
...
function goSearch(form,intPage){
    ...
    if(sForm.year_seq1.value       !="")parm+="&year_seq1="    + sForm.year_seq1.value; 
    if(sForm.doc_seq1.value        !="")parm+="&doc_seq1="     + escape(encodeURIComponent(sForm.doc_seq1.value));  
    if(sForm.center1.value         !="")parm+="&center1="      + sForm.center1.value;  
    if(sForm.run_dt1.value         !="")parm+="&run_dt1="      + sForm.run_dt1.value;     
    if(sForm.send1.value           !="")parm+="&send1="        +escape(encodeURIComponent(sForm.send1.value)); 
    if(sForm.title1.value          !="")parm+="&title1="       +escape(encodeURIComponent(sForm.title1.value)); 
    if(sForm.accach1.value         !="")parm+="&accach1="      +escape(encodeURIComponent(sForm.accach1.value)); 
    ...
    if(sForm.year_seq1.value       !="")sparam+="&s_year_seq1="    + sForm.year_seq1.value; 
    if(sForm.doc_seq1.value        !="")sparam+="&s_doc_seq1="     + escape(encodeURIComponent(sForm.doc_seq1.value));  
    if(sForm.center1.value         !="")sparam+="&s_center1="      + sForm.center1.value;  
    if(sForm.run_dt1.value         !="")sparam+="&s_run_dt1="      + sForm.run_dt1.value;     
    if(sForm.send1.value           !="")sparam+="&s_send1="        +escape(encodeURIComponent(sForm.send1.value)); 
    if(sForm.title1.value          !="")sparam+="&s_title1="       +escape(encodeURIComponent(sForm.title1.value)); 
    if(sForm.accach1.value         !="")sparam+="&s_accach1="      +escape(encodeURIComponent(sForm.accach1.value)); 
    if(sForm.remark1.value         !="")sparam+="&s_remark1="      +escape(encodeURIComponent(sForm.remark1.value));	
   	$('#list').jqGrid('clearGridData');
	jQuery("#list").setGridParam({url:"document.do?method=documentAccpList&parm="+parm}).trigger("reloadGrid");
}
...
function excelDown(){		
	...
	document.sForm.action="document.do?method=documentAccpDown&parm="+parm;
	document.sForm.submit();
}
...
<!-- 중복처리방지 토큰 처리  끝-->	  
	  ...
	  <table class="ctable_03" cellspacing="0" summary="문서">
		      ...
		      <tr>
               <td class="nobg" align="center">접수일</td>
               <td class="nobg1"><input type="text" name="receipt_dt1" id="datepicker" class="input" style=padding-top:3px  style=padding-bottom:3px  value="" size="12" align="center" /></td>
			   ...
			   <td class="nobg2" align="center">접수번호</td>
			   <td class="nobg1"><input type="text" id="year" name="year_seq1" size="15" disabled="true"/></td>               	   
             </tr>             
			 <tr>
               <td class="alt1" align="center">문서번호</td>
               <td colspan="0"><input type="text" id="year" name="doc_seq1" size="15" value="" /></td>
			    
			   <td class="alt" align="center">관리부서</td>
               <td width="24%">
                	...
                	for(int i=0;i<comcodesearch.size();i++){
               	    ...
               	    <!-- <input type="text" id="year" name="register1" size="10" /> -->
			   </td> 			                			  
               <td class="alt" colspan="1" align="center">시행일자</td>
               <td><input type="text" name="run_dt1" id="datepicker2" class="input" style=padding-top:3px  style=padding-bottom:3px value="" size="12" align="center" /></td>          			  
             </tr>
			 <tr>
               <td class="alt1" align="center">발신처</td>
               <td colspan="5"><input type="text" id="year" name="send1" size="147" value="" /></td>			   
             </tr>
			 <tr>
               <td  class="alt1" align="center">제목</td>               
                <td colspan="5"><input type="text" id="year" name="title1" size="147" value="" /></td>			   		  
             </tr>
			 <tr>
               <td  class="alt1" align="center">첨부물</td>               
                <td colspan="5"><input type="text" id="year" name="accach1" size="147" value="" /></td>			   		  
             </tr>
