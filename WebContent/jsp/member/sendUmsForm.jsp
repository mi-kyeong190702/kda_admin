...
function sendUms(form){		
	...
	if( "<%=code_pers%>".length > 0)	param+="&code_pers="+"<%=code_pers%>";
	...
	var url="";
// alert(param);
	if( from == "memberDues"	) url="memberDues.do?method=sendUmsLink"+param;
	if( from == "memberSearch"	) url="memberSearch.do?method=sendUmsLink"+param;
	form.action = url;
	form.submit();
}
...
<tr> 
	...
	<td>
		<%if(!"".equals(code_pers)){%>
		<input type="text" name="to"  value="<%=pers_name %>"  readonly size="25"/>
		<%}else{ %>
