...
.myAltRowClass {background-color:#f5f8f9; background-image:none;}
		...
		function init(){
			...
			var meprogid = "118"; //프로그램 ID ->dbo.prog_list
			var bcheck = "N";
			...
			$(document).ready(function(){
				...
				jQuery("#list").jqGrid({
					url: "memberState.do?method=certifiStateList",
						datatype: "json",
						mtype: 'POST',
						height:'500',
						width:'1100',
						colNames: [ '대분류','소분류','급식경영','임상영양','산업보건','합계'],
						colModel: [
		...
		<script>
			...
			function goSearch(form){
				...
				if(startdate > enddate){
					...
					jQuery("#list").setGridParam({url:"memberState.do?method=certifiStateList&startdate="+startdate+"&enddate="+enddate}).trigger("reloadGrid");  
				}		
			}
			function excelDown(){	
				if(document.memberStateInputForm.yn.value=='n'){
					...
					document.memberStateListForm.action="memberState.do?method=certifiStateDown";
		...
		<div class="mTabmenu_01"> 
			<div class="mTabmenu_01">
				...
				<input type="hidden" name="yn" value=""/>	
					    <table width=1100px class="mtable_13" cellspacing="0" summary="자격증소지별">
							<tr align="left">
								<!-- 기간 입력창 -->
