...
.myAltRowClass {background-color:#f5f8f9; background-image:none;}
		...
		function init(){
			...
			$(document).ready(function(){		
				jQuery("#list").jqGrid({
					url: "memberState.do?method=smallStateList",
						datatype: "json",
						mtype: 'POST',
						height:'500',
						width:'1100',
						colNames: [ '근무처구분','근무처대분류','근무처소분류','서울지부','부산지부','인천지부','경기지부','강원지부','충북지부','대전충남지부','전북지부','광주전남지부','대구경북지부','경남지부','울산지부','제주지부','기타','합계'],
						colModel: [
		...
		<script>
			function goSearch(form){
				...
				if(startdate > enddate){
					...
					jQuery("#list").setGridParam({url:"memberState.do?method=smallStateList&startdate="+startdate+"&enddate="+enddate}).trigger("reloadGrid"); 
				}	
			}
			...
			function excelDown(){	
				if(document.memberStateInputForm.yn.value=='n'){
					...
					document.memberStateListForm.action="memberState.do?method=smallStateDown";
		...
		<div class="mTabmenu_01"> 
			<div class="mTabmenu_01">
				...
				<input type="hidden" name="yn" value=""/>			
					    <table width=1100px class="mtable_13" cellspacing="0" summary="지부별 회원-소분류별">
							<tr align="left">
								<td class="a01" width=10%>구분</td>
								<td>
									<select name="gubun" onchange="javascript:cGubun(form);">
										<option value="1">분과별</option>
										<option value="2" selected>소분류별</option>
