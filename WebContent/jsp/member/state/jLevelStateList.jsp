...
.myAltRowClass {background-color:#f5f8f9; background-image:none;}
		...
		function init(){
			...
			var meprogid = "116"; //프로그램 ID ->dbo.prog_list
			var bcheck = "N";
			...
			$(document).ready(function(){
				...
				jQuery("#list").jqGrid({
					url: "memberState.do?method=jLevelStateList",
						datatype: "json",
						mtype: 'POST',
						height:'500',
						width:'1100',
						colNames: [ '대분류','소분류','5급','6급','7급','8급','9급','10급','무급','기타','합계'],
						colModel: [
		...
		<script>
			...
			function goSearch(form){
				...
				if(startdate > enddate){
					...
					jQuery("#list").setGridParam({url:"memberState.do?method=jLevelStateList&startdate="+startdate+"&enddate="+enddate}).trigger("reloadGrid"); 
				}
			}
			function excelDown(){	
				if(document.memberStateInputForm.yn.value=='n'){
					...
					document.memberStateListForm.action="memberState.do?method=jLevelStateDown";
