...
.myAltRowClass {background-color:#f5f8f9; background-image:none;}
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				//전체 권한 여부
				powerinit("112", eval(<%=userpowerm1%>) );


				
				jQuery("#list").jqGrid({
					url: "memberState.do?method=companyState",
						datatype: "json",
						mtype: 'POST',
						height:'500',
						width:'1100',
						...
						colModel: [
				{name:'detcodename'	,index:'detcodename asc, invdate', align:"center", width:100, sortable:false},
		...
		<script>
			...
			function goSearch(form){
				...
				jQuery("#list").setGridParam({url:"memberState.do?method=companyStateList&startdate="+startdate+"&enddate="+enddate}).trigger("reloadGrid"); 
...
function excelDown(form) {
	...
	form.target="frm";
	form.action="memberState.do?method=companyStateExcel"+param;
	form.submit();
	
	//jQuery("#list").setGridParam({url:"memberState.do?method=companyStateExcel"+param}).trigger("reloadGrid"); 
