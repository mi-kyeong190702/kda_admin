...
.myAltRowClass {background-color:#f5f8f9; background-image:none;}
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				//전체 권한 여부
				powerinit("122", eval(<%=userpowerm1%>) );


				
				jQuery("#list").jqGrid({
					url: "memberState.do?method=mealState",
						datatype: "json",
						mtype: 'POST',
						height:'500',
						width:'1100',
						colNames: [ '대분류','대분류','소분류','소분류','아침','점심','저녁','야식','합계'],
						colModel: [
		...
		<script>
			...
			function goSearch(form){
				...
				jQuery("#list").setGridParam({url:"memberState.do?method=mealStateList&startdate="+startdate+"&enddate="+enddate}).trigger("reloadGrid"); 
			...
			function excelDown(form) {
				...
				form.target="frm";
				form.action="memberState.do?method=mealStateExcel"+param;
				form.submit();
