...
.myAltRowClass {background-color:#f5f8f9; background-image:none;}
		...
		function init(){
			...
			var meprogid = "117"; //프로그램 ID ->dbo.prog_list
			var bcheck = "N";
			...
			$(document).ready(function(){
				...
				jQuery("#list").jqGrid({
					url: "memberState.do?method=jLineStateList",
						datatype: "json",
						mtype: 'POST',
						height:'500',
						width:'1100',
						colNames: [ '대분류','소분류','일용직','보건직','식품위생직','기능직','상용직','영양교사직','학교회계직','무직','기타','합계'],
						colModel: [
		...
		<script>
			...
			function goSearch(form){
				...
				if(startdate > enddate){
					...
					jQuery("#list").setGridParam({url:"memberState.do?method=jLineStateList&startdate="+startdate+"&enddate="+enddate}).trigger("reloadGrid");  
				}
			}
			function excelDown(){	
				if(document.memberStateInputForm.yn.value=='n'){
					...
					document.memberStateListForm.action="memberState.do?method=jLineStateDown";
