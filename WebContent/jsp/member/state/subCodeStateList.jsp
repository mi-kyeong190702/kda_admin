...
.myAltRowClass {background-color:#f5f8f9; background-image:none;}
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				//전체 권한 여부
				powerinit("121", eval(<%=userpowerm1%>) );


				
				jQuery("#list").jqGrid({
					url: "memberState.do?method=subCodeStateList",
						datatype: "json",
						mtype: 'POST',
						height:'500',
						width:'1100',
						colNames: [ '대분류','대분류','소분류','소분류','전국영양교사회','전국학교영양사회','전국교정영양사회','전국당뇨병영양사회','합계'],
						colModel: [
				...
				{name:'col1'		,index:'col3'			, width:100, align:"right", sortable:false, formatter:'integer'},
				{name:'col2'		,index:'col4'			, width:100, align:"right", sortable:false, formatter:'integer'},
		...
		<script>
			...
			function goSearch(form){
				...
				jQuery("#list").setGridParam({url:"memberState.do?method=subCodeStateList&startdate="+startdate+"&enddate="+enddate}).trigger("reloadGrid"); 
			...
			function excelDown(form) {
				...
				form.target="frm";
				form.action="memberState.do?method=subCodeStateExcel"+param;
				form.submit();
