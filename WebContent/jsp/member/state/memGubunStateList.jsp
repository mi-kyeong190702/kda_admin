...
.myAltRowClass {background-color:#f5f8f9; background-image:none;}
		...
		function init(){
			...
			$(document).ready(function(){		
				jQuery("#list").jqGrid({
					url: "memberState.do?method=memGubunStateList",
						datatype: "json",
						mtype: 'POST',
						height:'500',
						width:'1100',
						colNames: [ '회원구분1','회원구분2','회원구분3','서울지부','부산지부','인천지부','경기지부','강원지부','충북지부','대전충남지부','전북지부','광주전남지부','대구경북지부','경남지부','울산지부','제주지부','기타','합계'],
						colModel: [
				...
				{name:'groupname3'		,index:'groupname3'	,align:"center", width:1, sortable:false},
		...
		<script>
			function goSearch(form){
				...
				if(startdate > enddate){
					...
					jQuery("#list").setGridParam({url:"memberState.do?method=memGubunStateList&startdate="+startdate+"&enddate="+enddate}).trigger("reloadGrid"); 
				}			 
			}
			...
			function cGubun(form){
				...
				if(g==1){
					document.location.href="memberState.do?method=bigState";
				}else if(g==2){
			...
			function excelDown(){	
				if(document.memberStateInputForm.yn.value=='n'){
					...
					document.memberStateListForm.action="memberState.do?method=memGubunStateDown";
...
<body>
	<div id="contents">
		<ul class="home">
			...
			<li class="NPage">지부별 회원-회원종류별</li>
		 </ul>
		 <p>	
		<div class="mTabmenu_01"> 
			<div class="mTabmenu_01">
				...
				<input type="hidden" name="yn" value=""/>		
					    <table width=1100px class="mtable_13" cellspacing="0" summary="지부별 회원-회원종류별">
							<tr align="left">
								...
								<td>
									<select name="gubun" onchange="javascript:cGubun(form);">
										...
										<option value="3" selected>회원종류별</option>
									</select>
								</td>
								<!-- 기간 입력창 -->
				...
				<input type="hidden" name="enddate" value="">
					<!-- 엑셀출력 버튼 -->		
					<table width="1100" height="36" border="0" cellpadding="5" cellspacing="0" style="border-top:0px;">
						<tr>
