...
.myAltRowClass {background-color:#f5f8f9; background-image:none;}
		...
		function init(){
			...
			var meprogid = "119"; //프로그램 ID ->dbo.prog_list
			var bcheck = "N";
			...
			$(document).ready(function(){
				...
				jQuery("#list").jqGrid({
					url: "memberState.do?method=univerStateList",
						datatype: "json",
						mtype: 'POST',
						height:'500',
						width:'1100',
						colNames: [ '대분류','소분류','고졸','전문학사','학사','석사','박사','기타','합계'],
						colModel: [
		...
		<script>
			...
			function goSearch(form){
				...
				if(startdate > enddate){
					...
					jQuery("#list").setGridParam({url:"memberState.do?method=univerStateList&startdate="+startdate+"&enddate="+enddate}).trigger("reloadGrid");
				}	 
			}
			function excelDown(){	
				if(document.memberStateInputForm.yn.value=='n'){
					...
					document.memberStateListForm.action="memberState.do?method=univerStateDown";
