...
jQuery("#list").jqGrid({
      url: "memberBasic.do?method=postSearch&sel=Y",
      datatype: "json",
      mtype: 'POST',
      height:'230',
      width:'650',
					...
					if(ids == null) {
						...
						if(id){
							...
							if(pgu=='basic'){
								...
								opener.document.mem.m_address.value=ret.lnmAdres;
								opener.document.mem.d_address.value="";
								opener.document.getElementById("a_new").checked='true';
								self.close();
							}
							if(pgu=='comp'){
								opener.document.comp.cPost.value=ret.zipNo;
								opener.document.comp.cAddr.value=ret.lnmAdres;
...
function tsearch(){	
	...
	$('#list').jqGrid('clearGridData');
	jQuery("#list").setGridParam({url:"memberBasic.do?method=postSearchr&sel=Y&keyword="+keyword+"&searchSe="+searchSe}).trigger("reloadGrid"); 
}
...
<input type="hidden" name="register" value="<%=session.getAttribute("G_NAME") %>"/>
	<div id="p_contents">
	  <div id="ts_tabmenu1">
          <ul>
	        <li><a href="#" class="overMenu"><strong><span>도로명주소</span></strong></a></li>
	        <li><a href="memberBasic.do?method=postSearch1&sel=N&keyword=" ><strong><span>지번주소</span></strong></a></li>	        			
          </ul>
      </div>
