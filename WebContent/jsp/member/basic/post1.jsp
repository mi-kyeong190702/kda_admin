...
jQuery("#list").jqGrid({
      url: "memberBasic.do?method=postSearch1&sel=Y",
      datatype: "json",
      mtype: 'POST',
      height:'349',
      width:'650',
      colNames: ['우편번호','지번주소','성공여부','에러메세지'],
      colModel: [
			{name:'zipNo',			index:'zipNo',			width:30, 	sortable:false, align:'center'},			
			{name:'adres',		index:'adres', 		width:100, 	sortable:false},
				...
				onSelectRow: function(ids) {
					if(ids == null) {
						...
						if(id){
							...
							var pgu=opener.document.getElementById('pgu').value;
							if(pgu=='basic'){
								opener.document.mem.m_post.value=ret.zipNo;
								opener.document.mem.m_address.value=ret.adres;
								opener.document.mem.d_address.value="";
								opener.document.getElementById("a_old").checked='true';
								self.close();
							}
							if(pgu=='comp'){
								opener.document.comp.cPost.value=ret.zipNo;
								opener.document.comp.cAddr.value=ret.adres;
								opener.document.comp.cAddrd.value="";								
								self.close();
							}
						}
						else {}
...
function tsearch(){	
	...
	$('#list').jqGrid('clearGridData');
	jQuery("#list").setGridParam({url:"memberBasic.do?method=postSearchr1&sel=Y&keyword="+keyword+"&searchSe="+searchSe}).trigger("reloadGrid"); 
}
...
<input type="hidden" name="register" value="<%=session.getAttribute("G_NAME") %>"/>
	<div id="p_contents">
	  <div id="ts_tabmenu1">
          <ul>
	        <li><a href="memberBasic.do?method=postSearch&sel=N&keyword=" ><strong><span>도로명주소</span></strong></a></li>
	        <li><a href="#" class="overMenu"><strong><span>지번주소</span></strong></a></li>	        			
          </ul>
      </div>
	  <div class="mSearch_01">
        <table class="tbl_type" border="1" cellspacing="0" summary="우편번호 검색">      
            ...
            <tr>           
              <td>
	              <select name="searchSe" style="width:170px">	              		
	              		<option value="dong">동(읍/면)명/아파트/건물명</option>
	              		<option value="post">우편번호</option>
	              </select> 으로&nbsp;&nbsp;
             	  <input type="text" name="tName" size="13"  onkeydown="javascript:chkEnter();"/>&nbsp;<input type="button" name="btn" value="검색" onclick="javascript:tsearch();"></td>
            </tr>
            <!-- <tr>
