...
// 	  	String persno=(String)result.get(0).get("pers_no"); // JUMIN_DEL	 
	  	String url="memberBasic.do?method=certifiList&pCode="+pCode;
// 	  			+"&pers_no="+persno; // JUMIN_DEL
...
jQuery("#list").jqGrid({
      ...
      datatype: "json",
      mtype: 'POST',
      //height:'471', //근무처명, 근무처주소 들어갈땐 밑의 height으로 교체
      height:'436',
      width:'450',
      colNames: ['자격코드','자격증명','유효기간','유효시작일','유효종료일','본회','기타','총이수','이수여부'],
      colModel: [
			{name:'code_certifi',		index:'code_certifi', 		width:40, 	sortable:false, hidden:true},
			{name:'certifi_name',		index:'certifi_name', 		width:40, 	sortable:false},
			{name:'resultdt',			index:'resultdt', 			width:100, 	sortable:false},
   			{name:'result_start_dt',	index:'result_start_dt', 	width:50,	sortable:false, hidden:true},
   			{name:'result_end_dt',	index:'result_end_dt', 	width:50,	sortable:false, hidden:true},
   			{name:'col1',				index:'col1', 				width:50,	sortable:false},
   			{name:'col2',				index:'col2', 				width:50,	sortable:false},
   			{name:'total',				index:'total', 				width:50,	sortable:false},
   			{name:'print_state',		index:'print_state', 		width:40, 	sortable:false}   			
                ],
                rowNum:10000,
                /* pager: '#pager2', */
                grouping:true,
                groupingView:{
                	groupField : ['certifi_name'],
				...
				onSelectRow: function(ids) {
					if(ids == null) {
						...
						if(id){
							...
							var pCode=<%=pCode%>;
							var startdt=ret.result_start_dt;
							var enddt=ret.result_end_dt;
							var code_certifi = ret.code_certifi;
							jQuery("#list2").jqGrid('setGridParam',{url:"memberBasic.do?method=certifiList2&pCode="+pCode+"&startdt="+startdt+"&enddt="+enddt+"&code_certifi="+code_certifi}).trigger("reloadGrid");
						}
						else {}
					}
				}      
...
jQuery("#list2").jqGrid({
    url: "memberBasic.do?method=certifiList2",
    datatype: "json",
    mtype: 'POST',
    //height:'445', //근무처명, 근무처주소 들어갈땐 밑의 height으로 교체
    height:'410',
    width:'694',
    colNames: ['교육명','본회','이수일','평점','이수여부'],
    colModel: [
			{name:'edutest_name',	index:'edutest_name',	width:120, 	sortable:false},
			{name:'outside_yn',		index:'outside_yn', 		width:10, 	sortable:false, align:'center'},
			{name:'oper_end_dt',		index:'oper_end_dt', 		width:30, 	sortable:false, align:'center'},
 			{name:'result_point',		index:'result_point', 		width:10,	sortable:false, align:'center'},
 			{name:'result_state',		index:'result_state', 		width:30,	sortable:false, align:'center'} 			
              ],
              rowNum:10000,
              pager: '#pager3',             
              viewrecords: true,	
			  ...
			  onSelectRow: function(ids) {
					if(ids == null) {
						ids=0;
						if(jQuery("#list2").jqGrid('getGridParam','records') >0 ) //전체 레코드가 0보다 많다면
						{
							alert("컬럼을 정확히 선택해 주세요");
						}
					} else {
...
jQuery("#list2").jqGrid('navGrid','#page3',{edit:false,add:false,del:false,search:false,refresh:true});
});
</script>
</head>
<body>
	   ...
	   <div class="mTabmenu">
	    <div id="ts_tabmenu">
          <ul>
	        ...
	        <li><a href="#" class="overMenu"><strong><span>자격증</span></strong></a></li>
		...
		<div class="mTabmenu_01" id="tab02" >
