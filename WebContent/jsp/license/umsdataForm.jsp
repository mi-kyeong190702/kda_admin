...
<%
	...
	param += "&pers_hp=" + request.getParameter("pers_hp");	
	param += "&pers_name=" + request.getParameter("pers_name");
	param += "&isSelect="+  request.getParameter("isSelect");
	
	if( request.getParameter("from")  			!= null )	param+="&from="+request.getParameter("from");
	...
	if( request.getParameter("detcode")  	!= null )	param+="&detcode="+request.getParameter("detcode");
...
// 	JUMIN_DEL
// 	if( request.getParameter("oper_no1")  		!= null )	param+="&oper_no1="+request.getParameter("oper_no1");
	if( request.getParameter("oper_birth1")  		!= null )	param+="&oper_birth1="+request.getParameter("oper_birth1");
...
function fn_isrequest(form) {
	...
	function sendDms(){
	    ...
	    document.sForm.action = "license.do?method=umsData&parm="+parm+"<%=param%>";
	...
	function sendDmsall(){
		...
		var parm  ="&send_phone="		+  send_phone         +
		   			"&subject="			+  subject            +
		 			"&msg_body="		+  msg_body  +
		 			"&param="           +  '<%=param%>';
	    ...
	    //alert("전체 param="+parm); 	   
	    document.sForm.action = "license.do?method=umsDataall&parm="+parm;
	   	document.sForm.submit();
	   	//self.close();		
	}
...
<body onload="init();">
<form name="sForm" method="post" action="license.do?method=umsData" >

<table class="tbl_type" border="1" cellspacing="0" summary="회원검색" > 
	<tr> 
		<td align="center" >발송 대상</td>
		<td>
			<%if(("license_1".equals(from) || "license4".equals(from) || "license5".equals(from)) && request.getParameter("hp_infos")!=null && !"".equals(request.getParameter("hp_infos"))){ %>
			...
			<%if(("license_1".equals(from)||"license1".equals(from)||"license2".equals(from)||"license3".equals(from)||"license4".equals(from)||"license5".equals(from))){ %>
